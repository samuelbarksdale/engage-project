...
export default {
    listLectures: async function(req, res) {
        try {
            ...
            if (user.authority == 'ADMIN') {
                ...
                filter = {lectureName: user.lecture}
            }
    ...
    listCourse: async function(req, res) {
        //let did = req.session.cas.user;
        //let lec = await LectureModel.findOne({}).exec()
        let {id} = req.params;
        let lec = await LectureModel.findOne({}).exec();
        return res.json(lec.courseName);
    },
