...
export default {
    ...
    create: async function(req, res) {
        ...
        try {
            ...
            if ((await LectureModel.count().exec()) > 0) throw new ServerError("Please delete the saved course before creating another.")
            ...
            let userDocs = await Promise.all(roster.map(async ({name, uid, did, email, section}) => {
            ...
            await UserModel.update( {did: {$in: educators}}, {courseName});
            let lecturesDocs = await LectureModel.insertMany(lectureObjs);
            let sectionsDocs = await SectionModel.insertMany(sectionObjs);

            res.json({lectures: lecturesDocs, sections: sectionsDocs, users: userDocs});
        } catch (e) {
            console.error(e)
    ...
    course: async function(req, res) {
        let course = await LectureModel.findOne({}).select('courseName').exec();
        if (course) return res.json(course.courseName);
