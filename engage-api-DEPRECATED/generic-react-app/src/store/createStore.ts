import {ICourse, ISection, IScheduleMeeting, IUser, IPollQuestion} from '../common/model-definitions'
...
const absence_data: Record<string, IAbsence> = {
    1: {
        "id": "1",
        "uid": "sbarksda",
        "sections": ['10', '11'], 
        "type": "death",
        "rationale": "died from hackathon",
        "missed_days": new Date('2019-11-22'),
        file: ""
    }   
...
export class createStore {
    ...
    async loadCourses() {
        ...
        try {
            data = await (await fetch(config.courseURL + '/courses', {
                method: 'GET',
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
    ...
    async loadAbsences() {
        this.storeState = EStoreState.loading;
        let data = absence_data;
        let response:IAbsence[];
        console.log("here trying to fetch data from localhost data");
        try {
            response = await (await fetch('http://localhost:8081/absences', {
                method: 'GET',
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
    ...
    async insertCourse(
        ...
        let response;
        try {
            response = (await fetch(config.courseURL + '/courses', {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
